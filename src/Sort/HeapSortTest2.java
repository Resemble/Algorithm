...
public class HeapSortTest2 {
    ...
    public static void myMaxHeapSort(int[] array) {
        ...
        // 构建堆
        for (int i = length / 2 - 1; i >= 0 ; i--) {              // 比如 0 1 2 3 4 5 6 7 8，这9个数 8 / 2 - 1 = 3，只有 0 1 2 3 才有子节点
            adjustMaxHeapSort(array, i, length);
        }


        // 交换堆顶元素和最后一个元素，最后一个元素保持不变输出
        while(length >= 0) {
            swap(array, 0, length--); // 尾元素最大
            adjustMaxHeapSort(array, 0, length); // 再次调整堆
        }
